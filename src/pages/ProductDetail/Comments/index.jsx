...
const CommentsComponent = ({ productDetail }) => {
  ...
  const [commentsPaginCount, setcommentsPaginCount] = useState(0);
  ...
  const fetchParentComments = async (offset) => {
    try {

      const response = await axiosInstance.get(
        `/parent_comments/${productDetail.id}/?offset=${offset}&ordering=-created_at`
      );
        ...
        const updatedComments = commentsWithSubComments.map(newComment => {
          ...
          if (existingComment) {
            ...
            const updatedSubComments = newComment.sub_comments.filter(
              subComment => !existingComment.sub_comments.some(
            ...
            // Если родительский комментарий новый, добавляем его вместе с субкомментариями
            return newComment;
          }
        });
          ...
          JSON.stringify(
            {
              ...
              sender: {
                ...res.data.sender,
                avatar: res.data?.sender.avatar ? '' : res.data.sender.avatar
              },
  ...
  const handlePostSubComment = async (offset) => {
    if (!user_comment_sub) {
      toast.warning("Please write your comment");
      return;
    } else if (!user) {
  ...
  const formatDateTime = (CreateDate) => {
              ...
              <>
                {
                  comments.sort((a, b) => b.id - a.id).map((comment) => {
                    ...
                    return <div key={comment.id} className="mb-2">
                      <div className="d-flex align-items-start gap-3 " data-id={comment.id}>
                        ...
                        <div className="w-100">
                          ...
                          <form
                            ...
                            onSubmit={(e) => {
                              e.preventDefault();
                              handlePostSubComment(comment.offset);
                            }}
                          >
