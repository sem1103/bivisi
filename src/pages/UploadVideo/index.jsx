...
const UploadV = () => {
  ...
  const createCoverImg = () => {
    const video = document.getElementById("preview_video");
    

    if(video.src == window.location.href) video.src = URL.createObjectURL(file);
      ...
      const captureThumbnail = () => {
        if (captureIndex < captureTimes.length) {
          ...
          video.onseeked = async () => {
            ...
            canvas.toBlob((blob) => {
              blobs.push(blob)
              const coverImageFile = new File([blobs[localStorage.blobId]], `thumbnail${1}.jpg`, {
                type: "image/jpeg",
              });
              console.log(coverImageFile);
              console.log(blobs[localStorage.blobId]);
              setFormData((prevData) => ({
                ...prevData,
        ...
        <form onSubmit={handleSubmit} className="row pt-1">
          <div className="upload__video__form">
            ...
            <div className="form__content">
                        ...
                        <ul className="thumbernail__items">
                          {
                            thumbnail.map((item, ind) => {
                              return <li ><img onClick={() => {
                                localStorage.blobId = ind;
              ...
              <video id="preview_video" src=""></video>
            </div>
          </div>
