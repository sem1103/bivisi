...
const Shorts = () => {
  ...
  const activeShortId = localStorage.activeShort != undefined ? localStorage.activeShort :  videoProducts[Math.floor(Math.random() * videoProducts.length)]?.id;

  
  let videoProducts = product.results ? selectShortVideoHandler(product?.results?.filter((item) => item.product_video_type[0]?.product_type === "Shorts")) : [];
  ...
  function selectShortVideoHandler(array){
    let copyArray = [...videoProducts];
    let index = 0;
    array.forEach((item, ind) => {
        if(item.id == activeShortId) index = ind
      })
      return copyArray.unshift(copyArray.splice(index, 1)[0])
  }
