...
const Shorts = () => {
  ...
  const videoProducts = product?.results?.filter((item, index, self) => {
    if (item.product_video_type[0]?.product_type === 'Shorts') {
      if (item.id === activeShortId) {
        self.unshift(self.splice(index, 1)[0]);
        return item;
