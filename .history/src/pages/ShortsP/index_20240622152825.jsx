...
const Shorts = () => {
  ...
  const activeShortId = localStorage.activeShort != undefined ? localStorage.activeShort :  product?.results?.filter((item) => {
    if(item.product_video_type[0]?.product_type === "Shorts") return item
  })[Math.floor(Math.random() * videoProducts.length)]?.id;
